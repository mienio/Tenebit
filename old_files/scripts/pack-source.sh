...
for p in Tenebit.Backend Tenebit.Frontend .github scripts .gitignore .dockerignore docker-compose.production.yml SECURITY_INCIDENT_AUD9_001_RUNBOOK.md; do
...
find "$tmp/source" -type f \( -name '*.log' -o -name '*.log.*' -o -name '*.tsbuildinfo' -o -name '.env' -o -name 'appsettings.Production.json' \) -delete
find "$tmp/source" -type f -name '.env.*' ! -name '.env.example' -delete
