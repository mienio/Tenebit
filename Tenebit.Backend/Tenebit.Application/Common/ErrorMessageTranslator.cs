...
public static class ErrorMessageTranslator
{
    ...
    private static readonly Dictionary<string, Localized> Exact = new()
    {
        ...
        ["Brak aktywnej subskrypcji Stripe do zmiany - najpierw ją załóż przez płatność."] = new("There is no active Stripe subscription to change - set one up with a payment first.", "No hay una suscripción de Stripe activa para cambiar: primero configúrala con un pago.", "Es gibt kein aktives Stripe-Abonnement zum Ändern - richten Sie zuerst eines per Zahlung ein.", "Non c'è un abbonamento Stripe attivo da modificare: configuralo prima con un pagamento.", "Il n'y a pas d'abonnement Stripe actif à modifier - configurez-en un d'abord avec un paiement."),
        ["Płatność za zmianę planu nie powiodła się. Sprawdź metodę płatności w portalu rozliczeniowym Stripe i spróbuj ponownie."] = new("The payment for the plan change failed. Check your payment method in the Stripe billing portal and try again.", "El pago por el cambio de plan ha fallado. Comprueba tu método de pago en el portal de facturación de Stripe e inténtalo de nuevo.", "Die Zahlung für den Tarifwechsel ist fehlgeschlagen. Überprüfen Sie Ihre Zahlungsmethode im Stripe-Abrechnungsportal und versuchen Sie es erneut.", "Il pagamento per il cambio di piano non è riuscito. Controlla il metodo di pagamento nel portale di fatturazione Stripe e riprova.", "Le paiement du changement de forfait a échoué. Vérifiez votre moyen de paiement dans le portail de facturation Stripe et réessayez."),
